...
describe Users::PhoneConfirmationController, devise: true do
  ...
  describe '#send_code' do
    ...
    context 'when :unconfirmed_phone is set in session' do
      ...
      context 'confirmation code already exists in the session' do
        before do
          subject.user_session[:unconfirmed_sms_otp_delivery] = true
      ...
      context 'when phone is SMS OTP delivery enabled' do
        before { subject.user_session[:unconfirmed_sms_otp_delivery] = true }
        ...
        it 'does not send OTP via Voice', twilio: true do
          ...
          expect(VoiceSenderOtpJob).to_not receive(:perform_later).
            with('123456', '+1 (555) 555-5555')
      ...
      context 'when phone is not SMS OTP delivery enabled' do
        before { subject.user_session[:unconfirmed_sms_otp_delivery] = false }

        it 'sends confirmation code via Voice' do
          expect(subject).to receive(:confirmation_code).twice.and_return('123456')

          expect(VoiceSenderOtpJob).to receive(:perform_later).
            with('123456', '+1 (555) 555-5555')
  ...
  describe '#confirm' do
    before do
      ...
      subject.user_session[:unconfirmed_sms_otp_delivery] = false
    ...
    context 'user has an existing phone number' do
      context 'user enters a valid code' do
        ...
        it 'clears session data' do
          ...
          expect(subject.user_session[:unconfirmed_sms_otp_delivery]).to be_nil
          expect(subject.user_session[:phone_confirmation_code]).to be_nil
        end
      ...
      context 'user enters an invalid code' do
        ...
        it 'does not clear session data' do
          ...
          expect(subject.user_session[:unconfirmed_sms_otp_delivery]).to eq(false)
        ...
        it 'does not update user phone or phone_confirmed_at attributes' do
          ...
          expect(subject.current_user.sms_otp_delivery).to eq(true)
  ...
  describe '#disable_sms' do
    ...
    it 'sets session var unconfirmed_sms_otp_delivery to false' do
      get :disable_sms

      expect(subject.user_session[:unconfirmed_sms_otp_delivery]).to be(false)
    end
  ...
  describe '#enable_sms' do
    ...
    it 'sets session var unconfirmed_sms_otp_delivery to false' do
      ...
      expect(subject.user_session[:unconfirmed_sms_otp_delivery]).to be(true)
    end
