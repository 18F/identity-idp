...
feature 'Analytics Regression', js: true do
  ...
  context 'in person path' do
    ...
    before do
      ...
      allow_any_instance_of(Idv::InPerson::ReadyToVerifyPresenter).to receive(:service_provider_homepage_url).and_return(return_sp_url)
      allow_any_instance_of(Idv::InPerson::ReadyToVerifyPresenter).to receive(:sp_name).and_return(sp_friendly_name)
