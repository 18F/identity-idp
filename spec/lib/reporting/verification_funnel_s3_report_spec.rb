...
RSpec.describe Reporting::VerificationFunnelS3Report do
  let(:bucket_name) { 'test-bucket' }
  let(:custom_s3_path) { 'test/path/to/reports' }
  let(:agency_abbreviation) { 'IRS' }
  ...
  describe '#initialize' do
    ...
    it 'allows nil agency_abbreviation' do
      reader = Reporting::VerificationFunnelS3Report.new(
        bucket_name: bucket_name,
        custom_s3_path: custom_s3_path,
  ...
  describe '#csv_file_names' do
    # These must match the filenames uploaded by reporting-rails'
    # Reporting::VerificationFunnelReport#as_reports.
  ...
  describe '#csv_data_for' do
    ...
    context 'when S3 file does not exist' do
      ...
      it 'logs an error and re-raises the exception' do
        ...
        expect { report_reader.csv_data_for('definitions') }.to raise_error(
  ...
  describe '#get_file_last_modified' do
    ...
    it 'returns the last modified time for a file' do
      result = report_reader.get_file_last_modified('verification_funnel_metrics')

      expect(s3_client).to have_received(:head_object).with(
        bucket: bucket_name,
        key: "#{custom_s3_path}_verification_funnel_metrics.csv",
      )
  ...
  describe '#as_emailable_reports' do
    ...
    before do
      ...
      allow(report_reader).to receive(:verification_funnel_metrics_table).and_return(csv_data)
    end
    ...
    it 'sets correct titles and filenames' do
      ...
      expect(reports[2].title).to eq("#{agency_abbreviation} Verification Funnel Metrics")
      expect(reports[2].filename).to eq('verification_funnel_metrics')
    end
    ...
    context 'when agency_abbreviation is nil' do
      ...
      it 'excludes agency prefix from metric titles' do
        ...
        expect(reports[2].title).to eq('Verification Funnel Metrics')
      end
    end
  end
  ...
  describe 'individual table methods' do
    ...
    describe '#verification_funnel_metrics_table' do
      it 'returns CSV data for verification_funnel_metrics' do
        expect(report_reader.verification_funnel_metrics_table).to eq(csv_data)
        expect(report_reader).to have_received(:csv_data_for)
          .with('verification_funnel_metrics')
      end
    end
  end
end
