...
class SamlRequestedAttributesPresenter
  ...
  def identity_proofing_requested?
    if vtr.present?
      ...
      Vot::AuthnContextClassRefComponentValues.by_name[ial]&.requirements&.include?(
        :identity_proofing,
      )
    end
  end
  ...
  def ialmax_requested?
    Vot::AuthnContextClassRefComponentValues.by_name[ial]&.requirements&.include?(:ialmax)
  end
