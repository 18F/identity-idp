...
=======
  enum :document_type, {
    Idp::Constants::DocumentTypes::STATE_ID => 0,
    Idp::Constants::DocumentTypes::PASSPORT_BOOK => 1,
>>>>>>> 5dbe91ec5a (add document_type enum to document capture sessions model)
  }
  ...
  def store_result_from_response(doc_auth_response, attempt:, mrz_response: nil,
    ...
    if aamva_response
      session_result.source_check_vendor = aamva_response.extra[:vendor_name]
    elsif mrz_response
      session_result.source_check_vendor = mrz_response.extra[:vendor_name]
    end
    session_result.aamva_verified_attributes = aamva_response&.extra&.dig(:verified_attributes)
