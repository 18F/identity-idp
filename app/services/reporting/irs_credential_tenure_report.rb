...
module Reporting
  class IrsCredentialTenureReport
   ...
   def irs_credential_tenure_report_definition
      ...
      table << ["Credential Tenure", "The average age, in months, of all accounts measured from the account creation date to the last day of the month being reported."]
      table
    end
    ...
    def average_credential_tenure_months
      Reports::BaseReport.transaction_with_timeout do
        ...
        User.where('created_at <= ?', end_of_month).average(
          "EXTRACT(YEAR FROM age('#{end_of_month}', created_at)) * 12 + EXTRACT(MONTH FROM age('#{end_of_month}', created_at))",
        )&.round(2) || 0
      end
    end
  end
