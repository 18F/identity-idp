...
RSpec.describe Reporting::IrsVerificationReport do
  ...
  describe '#overview_table' do
    it 'generates the overview table with the correct data' do
      ...
      expect(table).to include(
        ...
        ['Report Generated', Time.zone.today.to_s], # Dynamically match today's date
        ['Issuer', issuers.join(', ')],
      )
    end
  end
  ...
  describe '#data_definition_table' do
    it 'generates the data definition table with the correct definitions' do
      table = report.data_definition_table
