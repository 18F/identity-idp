module Reports
  class AgencyInvoiceIaaSupplementReport < BaseReport
    ...
    def call
      raw_results =
        iaas.flat_map do |iaa|
          %i[sum unique new_unique].flat_map do |aggregate|
    ...
    # @return [Array<String>]
    def iaas
      ServiceProvider.distinct.where.not(iaa: nil).pluck(:iaa)
    end

    # Turns ial1/ial2 rows into ial1/ial2 columns
    def combine_by_iaa_month(raw_results)
      raw_results.group_by { |r| [r['iaa'], r['year_month']] }.transform_values do |grouped|
