module Reports
  class AgencyInvoiceIssuerSupplementReport < BaseReport
    ...
    def call
      raw_results =
        service_providers.flat_map do |service_provider|
    ...
    def service_providers
      ServiceProvider.where.not(iaa_start_date: nil).where.not(iaa_end_date: nil).to_a
    end

    # Turns ial1/ial2 rows into ial1/ial2 columns
    def combine_by_issuer_month(raw_results)
      raw_results.group_by { |r| [r['issuer'], r['year_month']] }.transform_values do |grouped|
