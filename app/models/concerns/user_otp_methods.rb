...
module UserOtpMethods
  ...
  def redis_direct_otp
    ...
    @redis_direct_otp = REDIS_POOL_NO_NAMESPACE.with do |client|
      client.get(
  ...
  def redis_direct_otp_expires_at
    ...
    time = @redis_direct_otp_expires_at = REDIS_POOL_NO_NAMESPACE.with do |client|
      client.expiretime(
        "user:direct_otp:#{self.id}",
      )
    end
  ...
  def set_redis_direct_otp(otp)
    ...
    REDIS_POOL_NO_NAMESPACE.with do |client|
      client.set(
        "user:direct_otp:#{self.id}",
        otp,
