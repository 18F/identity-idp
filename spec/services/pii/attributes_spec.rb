...
RSpec.describe Pii::Attributes do
  ...
  describe '#new_from_hash' do
    it 'initializes from plain Hash' do
      pii = Pii::Attributes.new_from_hash(first_name: 'Jane')
    ...
    it 'accepts Date values' do
      pii = Pii::Attributes.new_from_hash(dob: Date.new(2000, 1, 2))
  ...
  describe '#new_from_json' do
    it 'inflates from JSON string' do
      ...
      pii_attrs = Pii::Attributes.new_from_json(pii_json)

      expect(pii_attrs.first_name.to_s).to eq 'Jane'
    end

    it 'returns all-nil object when passed blank JSON' do
      expect(Pii::Attributes.new_from_json(nil)).to be_a Pii::Attributes
      expect(Pii::Attributes.new_from_json('')).to be_a Pii::Attributes
    end
  end

  describe '#==' do
    it 'treats objects with same values as equal' do
      pii_one = Pii::Attributes.new_from_hash(first_name: 'Jane')
      pii_two = Pii::Attributes.new_from_hash(first_name: 'Jane')
