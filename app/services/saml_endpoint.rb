class SamlEndpoint
  ...
  def secret_key
    ...
    OpenSSL::PKey::RSA.new(key_contents, endpoint_config[:secret_key_passphrase])
  end
  ...
  def saml_metadata
    ...
    SamlIdp::MetadataBuilder.new(config, x509_certificate, secret_key)
  end

  private

  def endpoint_config
    @endpoint_config ||= self.class.endpoint_configs.find { |config| config[:suffix] == suffix }
  end
  ...
  def suffix
    @suffix ||=
      begin
        ...
        suffixes.find { |suffix| request.path.match(/(metadata|auth|logout)#{suffix}$/) }
      end
  end
end
