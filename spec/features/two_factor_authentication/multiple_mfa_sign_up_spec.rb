...
RSpec.feature 'Multi Two Factor Authentication' do
  ...
  describe 'When the user has not set up 2FA' do
    ...
    scenario 'user can set up 2 MFA methods properly' do
      ...
      expect(page).to have_content t('headings.add_info.phone')
    ...
    scenario 'user can change method during setup' do
      sign_up_and_set_password
    ...
    scenario 'user can complete second MFA after reauthn window' do
    ...
    scenario 'user can select 1 MFA method and will be prompted to add another method' do
      sign_in_before_2fa
    ...
    describe 'skipping second mfa' do
      context 'with skippable mfa method' do
        it 'allows user to skip from backup code confirmation' do
          sign_up_and_set_password
          click_2fa_option('backup_code')
      ...
      context 'with platform authenticator as the first mfa' do
        it 'does not allow the user to skip selecting second mfa' do
          ...
          click_link t('mfa.add')
          expect(page).to have_current_path(authentication_methods_setup_path)
          expect(page).not_to have_button(t('forms.buttons.continue'))
        end
      end
    end
  end
