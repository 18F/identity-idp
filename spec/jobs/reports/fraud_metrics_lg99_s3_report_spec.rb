...
RSpec.describe Reporting::FraudMetricsLg99ReportS3 do
  ...
  let(:custom_s3_path) { 'sliang/idp/fraud-metrics-report/2025/2025-11-01.fraud-metrics-report' }
  ...
  before do
    ...
    s3_client.stub_responses(
      ...
      lambda do |context|
        ...
        when "#{custom_s3_path}/lg99_metrics.csv"
          { body: StringIO.new(lg99_metrics_csv) }
        when "#{custom_s3_path}/suspended_metrics.csv"
          { body: StringIO.new(suspended_metrics_csv) }
        when "#{custom_s3_path}/reinstated_metrics.csv"
  ...
  describe 'initialization' do
    ...
    context 'when report_date is provided without custom_s3_path' do
    ...
    context 'when custom_s3_path is explicitly provided' do
      it 'uses the given custom_s3_path (backward compatibility)' do
        ...
        expect(s3_client.api_requests.first).to include(
          ...
          params: hash_including(
            bucket: bucket_name,
            key: "#{custom_s3_path}/lg99_metrics.csv",
          ),
        )
      end
    end
  ...
  describe '#csv_data_for' do
    ...
    context 'when the S3 key does not exist' do
      it 'raises Aws::S3::Errors::NoSuchKey' do
        expect { report.csv_data_for('nonexistent_report') }.to raise_error(
          Aws::S3::Errors::NoSuchKey,
        )
      end

      it 'logs an error with bucket and key details' do
        expect(Rails.logger).to receive(:error).with(
          /CSV file not found in S3.*bucket=#{bucket_name}.*nonexistent_report\.csv/,
        )
        expect { report.csv_data_for('nonexistent_report') }.to raise_error(
          Aws::S3::Errors::NoSuchKey,
        )
