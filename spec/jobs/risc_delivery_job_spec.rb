...
RSpec.describe RiscDeliveryJob do
  ...
  describe '#perform' do
    ...
    shared_examples 'retries and logs event when attempts exhausted' do |exception|
      before do
        stub_request(:post, push_notification_url).to_raise(exception)
    ...
    it_behaves_like 'retries and logs event when attempts exhausted', Faraday::SSLError
    it_behaves_like 'retries and logs event when attempts exhausted', Faraday::ConnectionFailed
