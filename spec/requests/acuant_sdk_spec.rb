  ...
  context 'with a valid Acuant SDK asset' do
    [nil, *I18n.available_locales].each do |locale|
      context "with locale prefix: #{locale || '(none)'}" do
        %w[doc_auth capture-doc capture_doc].each do |verify_path|
          context "with verify path: #{verify_path}" do
            it 'renders a JS asset' do
              ...
              get "/#{locale}/verify/#{verify_path}/AcuantImageProcessingWorker.min.js"
            ...
            it 'renders a WASM asset' do
              get "/#{locale}/verify/#{verify_path}/AcuantImageProcessingWorker.wasm"
