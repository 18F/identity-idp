      ...
      context 'when checking ipp_aamva_result' do
        ...
        context 'when ipp_aamva_result is not present' do
          ...
          context 'when a passport is requested' do
            before do
              allow(subject.idv_session).to receive(:ipp_passport_requested?).and_return(true)
            end

            it 'returns true' do
              expect(
                described_class.step_info.preconditions.call(
                  idv_session: subject.idv_session, user:,
                ),
              ).to be(true)
            end
          end
