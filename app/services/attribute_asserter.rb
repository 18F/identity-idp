...
class AttributeAsserter
  ...
  def build
    attrs = default_attrs
    if bundle.include?(:email) && service_provider.emailaddress_attribute_enabled == true
      add_emailaddress(attrs)
    elsif bundle.include?(:email)
      add_email(attrs)
    end
