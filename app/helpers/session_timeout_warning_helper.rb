module SessionTimeoutWarningHelper
  def session_timeout_frequency
    (Identity::Hostdata.settings.session_check_frequency || 150).to_i
  end

  def session_timeout_start
    (Identity::Hostdata.settings.session_check_delay || 30).to_i
  end

  def session_timeout_warning
    (Identity::Hostdata.settings.session_timeout_warning_seconds || 30).to_i
  end
