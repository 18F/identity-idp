  ...
  def record_attempt(docv_result_response:, doc_pii_response: nil)
    ...
    attempts_api_tracker.idv_document_upload_submitted(
      ...
      **selfie,
      ...
      failure_reason: failure_reason(
        docv_result_response:, doc_pii_response:, image_errors:, tracker: attempts_api_tracker,
      ),
    )
    fraud_ops_tracker.idv_document_upload_submitted(
      ...
      **selfie,
      ...
      zip: pii_from_doc[:zipcode],
      failure_reason: failure_reason(
        docv_result_response:, doc_pii_response:, image_errors:, tracker: fraud_ops_tracker,
      ),
    )
  end

  def failure_reason(docv_result_response:, image_errors:, tracker:, doc_pii_response: nil)
    if doc_pii_response.present?
      failures = tracker.parse_failure_reason(doc_pii_response) || {}
    else
      failures = tracker.parse_failure_reason(docv_result_response) || {}
