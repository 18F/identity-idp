...
describe Profile do
  ...
  describe '#broken_personal_key?' do
    ...
    context 'for a profile that is not verified' do
      let(:profile) do
        create(:profile, activated_at: nil, verified_at: nil)
      end

      it 'is falsy' do
        expect(profile.broken_personal_key?).to be_falsey
      end
    end

    context 'for a profile verified before the broken key window' do
      let(:profile) do
        create(:profile, active: true, activated_at: 5.days.ago, verified_at: 5.days.ago)
      end
    ...
    context 'for a profile verified after the broken key window' do
      let(:profile) { create(:profile, :active, :verified, active: true) }
    ...
    context 'for a profile verified during the broken key window' do
      let(:personal_key_generated_at) { nil }
      let(:verified_at) { 2.days.ago }

      let(:user) do
        create(:user, encrypted_recovery_code_digest_generated_at: personal_key_generated_at)
      end
      let(:profile) do
        create(:profile,
          user: user,
      ...
      context 'for a user missing the personal key verified timestamp (legacy data)' do
        ...
        it 'is true' do
          expect(profile.broken_personal_key?).to eq(true)
        end
      end
      ...
      context 'for a personal key generated after the window (fixed)' do
        ...
        it 'is false' do
          expect(profile.broken_personal_key?).to eq(false)
        end
      end
    end
  end
