module Idv
  class CacController < ApplicationController
    ...
    def redirect_to_piv_cac_service
      Funnel::DocAuth::RegisterStep.call(current_user.id, 'present_cac', :update, true)
    ...
    def cac_callback
      ...
      analytics.track_event(analytics_submitted, result.to_h)
      if result.success?
        process_cac_success
      else
        process_cac_fail
    ...
    def process_cac_fail
      ...
      Funnel::DocAuth::RegisterStep.call(current_user.id, 'present_cac', :update, false)
    end
