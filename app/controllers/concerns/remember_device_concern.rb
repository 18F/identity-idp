module RememberDeviceConcern
  ...
  def pii_locked_for_session?(user)
    expired_for_interval?(
      user, Identity::Hostdata.settings.pii_lock_timeout_in_minutes.to_i.minutes
    )
  end
  ...
  def remember_device_cookie_expiration
    Identity::Hostdata.settings.remember_device_expiration_hours_aal_1.to_i.hours.from_now
  end
end
