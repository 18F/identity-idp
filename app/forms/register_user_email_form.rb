class RegisterUserEmailForm
  ...
  def send_sign_up_unconfirmed_email(request_id)
    throttler = RateLimit.new(user: existing_user, throttle_type: :reg_unconfirmed_email)
    throttler.increment!
  ...
  def send_sign_up_confirmed_email
    throttler = RateLimit.new(user: existing_user, throttle_type: :reg_confirmed_email)
    throttler.increment!
    @throttled = throttler.throttled?
