  ...
  class IncomingMetadata
    ...
    def name_id_formats
      xpath(
        ...
        md: metadata_namespace
      ).each_with_object(Set.new) do |el, set|
    ...
    def assertion_consumer_services
      xpath(
        ...
        md: metadata_namespace
      ).sort_by { |el| el['index'].to_i }.each_with_object([]) do |el, array|
    ...
    def given_name
      contact_person_document.xpath('//md:GivenName',
                                    md: metadata_namespace).first.try(:content).to_s
    end
    ...
    def telephone_number
      contact_person_document.xpath('//md:TelephoneNumber',
                                    md: metadata_namespace).first.try(:content).to_s
    end

    def email_address
      contact_person_document.xpath('//md:EmailAddress', md: metadata_namespace).first.try(:content).to_s.gsub(
        'mailto:', ''
      )
    end
