...
describe User do
  ...
  let(:all_methods) { UserTwoFactorMethods::METHODS }
  ...
  describe '#configuration_managers' do
    ...
    let(:two_factor_configurations) { user.two_factor_configurations }

    it 'returns one manager for each method' do
      expect(two_factor_configurations.count).to eq methods_count
      expect(all_methods - two_factor_configurations.map(&:method)).to eq []
      expect(two_factor_configurations.map(&:method) - all_methods).to eq []
    end
  end
  ...
  describe '#two_factor_enabled?' do
    context 'with piv/cac' do
      ...
      it 'returns true for piv/cac' do
        expect(user.two_factor_enabled?([:piv_cac])).to eq true
      end
      ...
      (UserTwoFactorMethods::METHODS - [:piv_cac]).each do |method|
        it "returns false for #{method}" do
          expect(user.two_factor_enabled?([method])).to eq false
        end

        it "returns true for [#{method}, piv_cac]" do
          expect(user.two_factor_enabled?([method, :piv_cac])).to eq true
        end
      end
    end
  end
  ...
  describe '#two_factor_configurable?' do
    context 'with no phone configured' do
      ...
      it 'returns true that some method is configurable' do
        expect(user.two_factor_configurable?).to eq true
      end

      it 'returns true that sms or voice is configurable' do
        expect(user.two_factor_configurable?(%i[sms voice])).to eq true
      end
    end
    ...
    context 'with phone configured' do
      ...
      it 'returns false that sms or voice is configurable' do
        expect(user.two_factor_configurable?(%i[sms voice])).to eq false
      end
    end
    ...
    context 'with all methods configured' do
      ...
      it 'returns false that some method is configurable' do
        expect(user.two_factor_configurable?).to eq false
      end
    end
  end

  describe '#configurable_configuration_managers' do
    let(:configurable_methods) { user.two_factor_configurable_method_configurations.map(&:method) }
  ...
  describe '#configuration_manager' do
    ...
    UserTwoFactorMethods::METHODS.each do |method|
      it "creates a configuration manager for #{method}" do
        manager = subject.two_factor_configuration(method)
        expect(manager).to be_a(TwoFactorAuthentication::MethodConfiguration)
