...
RSpec.describe Health::HealthController do
  before do
    allow(IdentityConfig.store).to receive(:job_run_healthchecks_enabled).and_return(true)
  end
  ...
  describe '#index' do
    ...
    context 'job run healthchecks are disabled' do
      it 'does not include job run healthchecks' do
        allow(IdentityConfig.store).to receive(:job_run_healthchecks_enabled).and_return(false)
