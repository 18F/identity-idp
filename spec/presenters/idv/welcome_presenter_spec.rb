...
RSpec.describe Idv::WelcomePresenter do
  subject(:presenter) { Idv::WelcomePresenter.new(decorated_sp_session, id_type_policy) }
  ...
  let(:id_type_policy) { instance_double(Idv::IdTypePolicy, allow_passport?: false) }
  ...
  describe 'the bullet points' do
    ...
    context 'when the user is allowed to use a passport' do
      let(:id_type_policy) { instance_double(Idv::IdTypePolicy, allow_passport?: true) }
