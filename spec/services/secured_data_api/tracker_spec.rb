...
RSpec.describe SecuredDataApi::Tracker do
  before do
    allow(IdentityConfig.store).to receive(:secured_data_api_enabled)
      .and_return(secured_data_api_enabled)
  ...
  let(:secured_data_api_enabled) { true }
  ...
  describe '#track_event' do
    ...
    it 'records the event in redis' do
      freeze_time do
        ...
        events = SecuredDataApi::RedisClient.new.read_events(
    ...
    context 'user has existing Agency UUID' do
      context 'for event that skips UUID creation' do
        it 'returns Agency UUID in event' do
          ...
          event = subject.track_event(
            SecuredDataApi::Tracker::SKIP_AGENCY_UUID_CREATION_EVENT_TYPES.sample,
          )
    ...
    context 'the current session is not a secured data API session' do
    ...
    context 'the secured data API is not enabled' do
      let(:secured_data_api_enabled) { false }
