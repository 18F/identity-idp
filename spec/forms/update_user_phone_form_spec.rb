...
describe UpdateUserPhoneForm do
  ...
  describe 'OTP delivery preference' do
    ...
    context 'when SMS disabled and enabling SMS' do
      let(:user) { build_stubbed(:user, :signed_up, phone_sms_enabled: false) }
      let(:params) do
        { phone: user.phone,
          phone_sms_enabled: '1' }
      end
    ...
    context 'when not changing SMS preference' do
      let(:user_sms) { build_stubbed(:user, :signed_up, phone_sms_enabled: true) }
      let(:user_no_sms) { build_stubbed(:user, :signed_up, phone_sms_enabled: false) }
