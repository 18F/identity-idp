class CreateInPersonEnrollments < ActiveRecord::Migration[6.1]
  def change
    create_table :in_person_enrollments do |t|
      t.references :user, null: false, foreign_key: true, comment: 'Foreign key to the user this enrollment belongs to'
      t.references :profile, null: false, foreign_key: true, comment: 'Foreign key to the profile this enrollment belongs to'
