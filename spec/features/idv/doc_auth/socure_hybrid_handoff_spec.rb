...
RSpec.feature 'Socure hybrid_handoff step send link and errors' do
  ...
  before do
    ...
    allow(IdentityConfig.store).to receive(:socure_document_request_endpoint).and_return('https://service.socure.us/api/5.0/documents/request')
    allow(IdentityConfig.store).to receive(:socure_id_plus_secret_key).and_return('silly_key')
    stub_request(:post, "https://service.socure.us/api/5.0/documents/request").
...
RSpec.feature 'Socure hybrid_handoff step for ipp, selfie variances', js: true do
  include IdvStepHelper
  include DocAuthHelper
