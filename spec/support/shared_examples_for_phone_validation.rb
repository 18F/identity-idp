...
shared_examples 'a phone form' do
  ...
  describe 'phone uniqueness' do
    context 'when phone is already taken' do
      it 'is valid' do
        ...
        allow(User).to receive(:exists?).with(
          phone_configuration: { phone: second_user.mfa.phone_configurations.first.phone }
        ).and_return(true)

        params[:phone] = second_user.mfa.phone_configurations.first.phone
    ...
    context 'when phone is same as current user' do
      it 'is valid' do
        user.mfa.phone_configurations.first.phone = '+1 (703) 500-5000'
