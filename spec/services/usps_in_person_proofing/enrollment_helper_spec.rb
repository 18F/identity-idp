...
RSpec.describe UspsInPersonProofing::EnrollmentHelper do
  ...
  let(:usps_ipp_applicant_document_info_enabled) { false }
  ...
  before(:each) do
    allow(IdentityConfig.store).to receive(:usps_ipp_applicant_document_info_enabled)
      .and_return(usps_ipp_applicant_document_info_enabled)
    ...
    context 'when the user has an establishing InPersonEnrollment' do
      ...
      context 'when in-person mocking is not enabled' do
        ...
        context 'when the USPS enrollment creation is successful' do
          ...
          context 'when include ID for USPS IPP applicants is disabled' do
            let(:usps_ipp_applicant_document_info_enabled) { false }
          ...
          context 'when include ID for USPS IPP applicants is enabled' do
            let(:usps_ipp_applicant_document_info_enabled) { true }
