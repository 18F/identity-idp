...
RSpec.describe Proofing::Mock::IdMockClient do
  ...
  describe '#proof' do
    ...
    context 'with a passport' do
      let(:applicant) { Idp::Constants::MOCK_IDV_APPLICANT_WITH_PASSPORT.merge(uuid: '1234-abcd') }
      ...
      context 'with an invalid mrz' do
        it 'returns a failed to match result' do
          ...
          expect(result.success?).to eq(false)
          expect(result.errors).to eq(
            mrz: ['The passport MRZ could not be verified'],
          )
          expect(result.transaction_id).to eq(transaction_id)
          expect(result.to_h).to eq(
            success: false,
