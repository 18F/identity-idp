...
feature 'phone otp rate limiting', :idv_job do
  ...
  describe 'otp sends' do
    let(:max_attempts) { Identity::Hostdata.settings.otp_delivery_blocklist_maxretry.to_i + 1 }
  ...
  def expect_rate_limit_to_expire(user)
    ...
    retry_minutes = Identity::Hostdata.settings.lockout_period_in_minutes.to_i + 1
