module VerifySpAttributesConcern
  def needs_completions_screen?
    sp_session[:issuer].present? &&
      (
        sp_session_identity.nil? || !requested_attributes_verified? || consent_has_expired? ||
          consent_was_revoked?
      )
  end
  ...
  def update_verified_attributes
    IdentityLinker
      .new(current_user, sp_session[:issuer])
      .link_identity(
        ial: sp_session_ial,
  ...
  def new_service_provider_attributes
    if user_session.class == ActiveSupport::HashWithIndifferentAccess
      user_session[:verify_shared_attributes]
    end
  end
  ...
  def sp_session_identity
    @sp_session_identity = current_user&.identities&.find_by(service_provider: sp_session[:issuer])
  end
