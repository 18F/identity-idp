...
module SecuredDataApi
  class SecuredDataEvent
    ...
    def self.from_jwe(jwe, private_key)
      ...
      if IdentityConfig.store.secured_data_api_signing_enabled
    ...
    def long_event_type
      ...
      "https://schemas.login.gov/secevent/secured-data-api/event-type/#{dasherized_name}"
    end
    ...
    module SigningKey
      ...
      def self.signing_key
        raise SigningKeyError, 'Secured Data API signing key is not configured' if
          IdentityConfig.store.secured_data_api_signing_key.blank?

        OpenSSL::PKey::EC.new(IdentityConfig.store.secured_data_api_signing_key)
      end
    end
  end
end
