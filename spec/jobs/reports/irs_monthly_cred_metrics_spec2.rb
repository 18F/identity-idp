...
RSpec.describe Reports::IrsMonthlyCredMetricsReport do
  ...
  let(:name) { 'monthly-credentials-metrics-report' }
  let(:s3_report_bucket_prefix) { 'reports-bucket' }
  let(:report_folder) do
    'int/monthly-credentials-metrics-report/2021/2021-03-02.monthly-credentials-metrics-report'
  end

  let(:expected_s3_paths) do
    [
      "#{report_folder}/credentials_authorized_requesting_agency.csv",
      "#{report_folder}/new_identity_verification_credentials_authorized.csv",
      "#{report_folder}/existing_identity_verification_credentials_authorized.csv",
    ]
  end
  ...
  context 'when queued from the first of the month' do
    ...
    it 'sends out a report to everybody' do
      expect(ReportMailer).to receive(:tables_report).once.with(
        email: anything,
        subject: 'Monthly Credentials Metrics Report - 2021-02-28',
        reports: anything,
        message: report.preamble,
        attachment_format: :xlsx,
      ).and_call_original
