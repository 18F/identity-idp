...
class UuidReporter
  ...
  def all_issuers_belong_to_same_agency?
    agency_count =
      Agency.joins(:service_providers).where(service_providers: { issuer: issuers }).distinct.count
  ...
  def find_agency
    Agency.joins(:service_providers).find_by(service_providers: { issuer: issuers.first })
  end
  ...
  def save_csv(emails_to_uuids)
    CSV.open(output, 'w') do |csv|
      csv << %w[email_address uuid]

      emails_to_uuids.each { |email, uuid| csv << [email, uuid] }
    end
  end
end
