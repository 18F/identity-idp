...
const helpCenterUrl = 'https://login.gov/help/verify-your-identity/verify-your-identity-in-person/find-a-participating-post-office';

export default function UspsFacilitiesApiErrorMessage() {
  const { formatHTML } = useI18n();

  const translatedErrorBody = t(
    'in_person_proofing.body.location.po_search.usps_facilities_api_error_body',
  ...
  function formatErrorBody() {
    return formatHTML(
      translatedErrorBody, {
        'a': () => <Link href={helpCenterUrl} isExternal>{t('in_person_proofing.body.location.po_search.usps_facilities_api_error_help_center_text')}</Link>
      }
    );
  }
  ...
  return (
    <div className="usps-facilities-api-error grid-row">
      ...
      <div className="usps-facilities-api-error__text grid-col-12 tablet:grid-col-fill">
        ...
        <p className="usps-facilities-api-error__body margin-y-0">
          {formatErrorBody()}
        </p>
      </div>
    </div>
