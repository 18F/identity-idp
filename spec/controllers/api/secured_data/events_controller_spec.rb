...
RSpec.describe Api::SecuredData::EventsController do
  ...
  describe '#poll' do
    ...
    let(:redis_client) { SecuredDataApi::RedisClient.new }
    ...
    before do
      ...
      allow(IdentityConfig.store).to receive(:allowed_secured_data_providers).and_return(
        [{
          'issuer' => sp.issuer,
          'tokens' => [{ 'value' => hashed_token, 'salt' => salt, 'cost' => cost }],
        }],
      )
      allow(SecuredDataApi::RedisClient).to receive(:new).and_return redis_client
    end
    ...
    context 'when the Secured Data API is enabled' do
      ...
      context 'with a valid authorization header' do
        ...
        context 'with events stored in redis' do
          ...
          let(:event) do
            SecuredDataApi::SecuredDataEvent.new(
          ...
          it 'returns a json blob including that set' do
            ...
            expect(@analytics).to have_logged_event(
              :secured_data_api_poll_events_request,
              issuer: issuer,
