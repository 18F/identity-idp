...
describe Idv::UspsMail do
  ...
  describe '#mail_spammed?' do
    ...
    context 'when the amount of sent mail is lower than the allowed maximum' do
      it 'returns false' do
        event_create(event_type: :usps_mail_sent, user: user)
    ...
    context 'when too much mail has been sent' do
      it 'returns true if the oldest event was within the last month' do
        event_create(event_type: :usps_mail_sent, user: user, updated_at: 2.weeks.ago)
        event_create(event_type: :usps_mail_sent, user: user, updated_at: 1.week.ago)
      ...
      it 'returns false if the oldest event was more than a month ago' do
        event_create(event_type: :usps_mail_sent, user: user, updated_at: 2.weeks.ago)
        event_create(event_type: :usps_mail_sent, user: user, updated_at: 2.months.ago)
  ...
  def event_create(hash)
    ...
    DeviceEvent.create(device_id: device.id,
                       ip: remote_ip,
                       event_type: event,
