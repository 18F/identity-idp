...
RSpec.describe SecuredDataApi::SecuredDataEvent do
  ...
  describe '#to_jwe' do
    describe 'secured data event signing is enabled' do
      before do
      ...
      describe 'when the secured data signing key is present' do
        ...
        it 'returns a JWE for the event' do
          jwe = subject.to_jwe(
            ...
            public_key: secured_data_api_public_key,
          )
          ...
          expect(headers['kid']).to eq(JWT::JWK.new(secured_data_api_public_key).kid)

          decrypted_jwe_payload = JWE.decrypt(jwe, secured_data_api_private_key)
          ...
          event_key = 'https://schemas.login.gov/secevent/secured-data-api/event-type/test-event'
    ...
    describe 'secured data event signing is not enabled' do
      it 'returns a JWE for the event' do
        jwe = subject.to_jwe(
          issuer: service_provider.issuer,
          public_key: secured_data_api_public_key,
  ...
  describe '.from_jwe' do
    describe 'when secured data signing is not enabled' do
      it 'returns an event decrypted from the JWE' do
        ...
        decoded_event = described_class.from_jwe(jwe, secured_data_api_private_key)
    ...
    describe 'when secured data signing is enabled' do
      ...
      describe 'when the secured data signing key is not present' do
        ...
        it 'raises an error' do
          expect do
            subject.to_jwe(issuer: service_provider.issuer, public_key: secured_data_api_public_key)
          end.to raise_error(
            SecuredDataApi::SecuredDataEvent::SigningKey::SigningKeyError,
            'Secured Data API signing key is not configured',
          )
