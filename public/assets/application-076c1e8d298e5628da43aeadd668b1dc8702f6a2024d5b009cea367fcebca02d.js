...
$(document).on('page:change', function () {
  $('#start_quiz_form').on('submit',  function () {
    $('.bottom-section').append('<div class="generating" tabindex="-1"><h4>Generating Quiz</h4>' +
      '<p>This can take up to a minute. We will start your quiz when it is ready.</p></div>');
    $('#start_quiz').addClass('spinner');
...
$(document).on('page:change', function () {
  ...
  $('#help-button').click(function () {
    if ($('#help-content').hasClass('open')) {
      $('#help-content').removeClass('open');
      $('#help-content').slideUp(slideTime);
      $('#help-label').html("Where do I find this?<span class='sr-only'>– Display document sample below</span>");
    } else {
      $('#help-content').addClass('open');
      $('#help-content').slideDown(slideTime);
      $('#help-label').html("Hide document <span class='sr-only'>sample displayed below</span>");
      $('#help-content').removeClass('slide-panel');
    }
  });
});
$(document).on('page:change', function () {
