...
RSpec.describe EncryptedDocStorage::S3Storage do
  ...
  let(:profile_id) { '3' }
  let(:path) { "#{user_uuid}/#{profile_id}/#{name}" }
  let(:encrypted_attempt_events) { SecureRandom.uuid }
  ...
  describe '#retrieve_attempt_object' do
    before do
      ...
      response = double(body: StringIO.new(encrypted_attempt_events))
