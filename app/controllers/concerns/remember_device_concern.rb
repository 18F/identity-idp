module RememberDeviceConcern
  ...
  def check_remember_device_preference
    ...
    unless remember_device_cookie.valid_for_user?(
             user: current_user,
  ...
  def remember_device_cookie
    ...
    @remember_device_cookie ||= RememberDeviceCookie.from_json(remember_device_cookie_contents)
  end

  def remember_device_expired_for_sp?
    expired_for_interval?(current_user, decorated_session.mfa_expiration_interval)
  end
  ...
  def revoke_remember_device(user)
    UpdateUser.new(user: user, attributes: { remember_device_revoked_at: Time.zone.now }).call
  end
  ...
  def expired_for_interval?(user, interval)
    ...
    !remember_cookie.valid_for_user?(user: user, expiration_interval: interval)
  end
