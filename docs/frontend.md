...
to produce a readable stack trace of the original code. Sourcemaps are not generated by default in
production builds, but you can build these in your local development environment.

First, checkout the current production commit, assuming that the error is recent:
...
Then, build the JavaScript assets as a production artifact with sourcemaps enabled:

```
NODE_ENV=production WEBPACK_DEVTOOL=source-map yarn build
```

Once built, locate the `.map` file in the `assets/packs/js/` directory corresponding to the file
mentioned in the minified stack trace and drag-and-drop the file onto the NewRelic interface to
reveal the decompiled stack trace.
