...
function PostOfficeNoResultsMock({ address }) {
  return (
...
describe('InPersonLocations', () => {
  context('when no locations are found', () => {
    it('renders the passed in postOfficeNoResults component w/ address', () => {
      ...
      const { getByText } = render(
        <InPersonLocations
          ...
          postOfficeNoResults={PostOfficeNoResultsMock}
        />,
      );
  ...
  context('when at least 1 location is found', () => {
    it('renders a list of Post Offices and does not render the passed in postOfficeNoResults component', () => {
      const onClick = sinon.stub();
      const mockLocation = [
        {
          name: 'test name',
