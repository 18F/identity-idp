...
describe TwoFactorAuthentication::WebauthnPlatformSelectionPresenter do
  ...
  let(:configuration) {}
  let(:presenter_without_mfa) { described_class.new(configuration: configuration, user: user_without_mfa) }
  let(:presenter_with_mfa) { described_class.new(configuration: configuration, user: user_with_mfa) }
