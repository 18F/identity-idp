...
RSpec.describe 'OpenID Connect using vectors of trust', allowed_extra_analytics: [:*] do
  include IdvHelper
  ...
  xcontext 'with client_secret_jwt prompt login' do
  ...
  xcontext 'with client_secret_jwt prompt select account' do
    ...
    it 'succeeds in returning back to sp with prompt select_account and prior session' do
      ...
      expect(oidc_redirect_url).to include('?code=')
    end
    ...
    context 'when using prompt=login' do
      ...
      it 'does show reauthentication notice if user was actively authenticated' do
        ...
        visit_idp_from_oidc_sp_with_vtr(vtr: 'C1', prompt: 'login')
      ...
      it 'does not show reauth notice if most recent request in session was not prompt=login' do
        ...
        visit_idp_from_oidc_sp_with_vtr(vtr: 'C1', prompt: 'select_account')
  ...
  context 'logging into an SP for the first time' do
    it 'displays shared attributes page once' do
      ...
      oidc_path = visit_idp_from_oidc_sp_with_vtr(vtr: 'C1', prompt: 'select_account')
      sign_in_live_with_2fa(user)
  ...
  def sign_in_get_token_response(
    ...
    redirect_uri = if current_path.start_with?('/openid_connect/')
