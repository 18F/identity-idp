...
describe('DocumentCaptureReviewIssues', () => {
  ...
  context('with selfie enabled', () => {
    it('renders alternative content', () => {
      ...
      const idTipListHeader = getByText('doc_auth.tips.document_capture_selfie_id_header_text');
      expect(idTipListHeader).to.be.ok();
      const lists = getAllByRole('list');
      const tipList = lists[0];
      expect(tipList).to.be.ok();
      const tipListItem = within(tipList).getAllByRole('listitem');
      tipListItem.forEach((li, idx) => {
        expect(li.textContent).to.equals(`doc_auth.tips.document_capture_id_text${idx + 1}`);
      });
    });
  });
});
