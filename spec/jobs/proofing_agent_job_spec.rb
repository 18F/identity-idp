...
RSpec.describe ProofingAgentJob, type: :job do
  ...
  describe '#perform' do
    ...
    context 'when state_id expiration is near (<2 days)' do
      let(:pii) do
        Idp::Constants::MOCK_IDV_APPLICANT_SAME_ADDRESS_AS_ID_WITH_PHONE.merge(
          state_id_expiration: (Time.zone.today + 2.days).to_s,
        )
      end
      ...
      it 'stores a failed result' do
        ...
        result = document_capture_session.reload.load_agent_proofed_user
        expect(result[:success]).to be false
        expect(result[:reason]).to eq('expiration_date_near')
      end
      ...
      it 'does not call AAMVA verification' do
        expect(instance).not_to receive(:call_aamva_verification)
        perform
      end
    ...
    context 'when passport expiration is near (<2 days)' do
      let(:pii) do
        Idp::Constants::MOCK_IDV_PROOFING_PASSPORT_APPLICANT.merge(
          passport_expiration: (Time.zone.today + 2.days).to_s,
          phone: '12025551212',
        ).freeze
      end
      ...
      it 'does not call MRZ verification' do
        expect(instance).not_to receive(:call_mrz_verification)
        perform
      end

      it 'does not log idv_dos_passport_verification' do
        perform
        expect(@analytics).not_to have_logged_event('IdV: dos passport verification')
      end
    end
