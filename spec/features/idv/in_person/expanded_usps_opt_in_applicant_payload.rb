...
RSpec.describe 'In Person Proofing: Expanded opt in applicant', js: true do
  ...
  context 'When the usps_ipp_applicant_document_info_enabled is true' do
    before do
      allow(IdentityConfig.store).to receive(:usps_ipp_applicant_document_info_enabled).and_return(
        true,
      )
    end
          ...
          expect(JSON.parse(req.body)).to eq(
            {
              ...
              'emailAddress' => IdentityConfig.store.usps_ipp_enrollment_status_update_email_address,
  ...
  context 'When the in_person_usps_applicant_document_info_enabled is false' do
    before do
      allow(IdentityConfig.store).to receive(:usps_ipp_applicant_document_info_enabled).and_return(
        false,
      )
