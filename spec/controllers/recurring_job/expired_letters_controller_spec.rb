...
describe RecurringJob::ExpiredLettersController do
  describe '#create' do
    it_behaves_like 'a recurring job controller',
                    Identity::Hostdata.settings.expired_letters_auth_token

    context 'with a good auth token' do
      before do
        request.headers['X-API-AUTH-TOKEN'] = Identity::Hostdata.settings.expired_letters_auth_token
      end
