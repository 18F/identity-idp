...
describe Acuant::Liveness do
  ...
  let(:not_live_api_body) do
    { 'LivenessResult' => { 'Score' => 99, 'LivenessAssessment' => 'NotLive' }, 'Error' => nil,
      'ErrorCode' => nil, 'TransactionId' => '4a11ceed-7a54-45fa-9528-3945b51a1e23' }
  end
  let(:bad_liveness_result) do
    [true, not_live_api_body.to_json]
  end
  ...
  it 'fails liveness and returns the api body if the selfie is not live' do
    allow_any_instance_of(Idv::Acuant::Liveness).to receive(:liveness).
      and_return(bad_liveness_result)
    allow_any_instance_of(Idv::Acuant::AssureId).to receive(:face_image).
      and_return(good_faceimage_result)
    allow_any_instance_of(Idv::Acuant::FacialMatch).to receive(:facematch).
      and_return(good_facematch_result)
    ...
    expect(result).to eq([false, not_live_api_body])
  end
end
