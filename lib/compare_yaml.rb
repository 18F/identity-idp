...
class CompareYaml
  # This is a black box end-to-end test spitting out the results and checking
  # for regressions trying to replicate as close as possible the production database
  # Before you can run this you must have identity-idp-config repo set up and run serialized.rb
  # Serialized.rb script will create a 'serialized' folder which needs to be copied to this repo
  # Files from config should be copied into the identity-idp config folder
  # replacing the existing yml files which only have test data
    ...
    value.each_with_index do |item, index|
      if (prod[index] != item)
        ...
        if account_name_hash.nil?
          @diff[symbol] << ({ item['partner_account_name'] => [item.to_a - prod[index].to_a] })
         else
  ...
  def objects
    {
      service_providers: ServiceProvider.all.each_with_object({}) do |sp, object|
                           object[sp[:issuer].to_s] =
                             sp.attributes.except(
                               'id', 'created_at', 'updated_at',
                               'irs_attempts_api_enabled'
                             )
                         end,
      integrations: Agreements::Integration.all.each_with_object({}) do |int, object|
                      object[int[:issuer].to_s] =
                        int.attributes.except(
                          'id', 'partner_account_id', 'integration_status_id',
                          'service_provider_id'
                        )
                    end,
      iaa_orders: Agreements::IaaOrder.all.includes(:partner_account).
        each_with_object({}) do |order, object|
                    # id values are randomly generated and dont match across the instances
                    partner_name = { 'partner_account_name' => order.partner_account.name }
                    object[order.partner_account.name] = [] if !object[order.partner_account.name]
                    object[order.partner_account.name] << order.attributes.except(
                      'id',
                      'iaa_gtc_id',
                    ).merge(partner_name)
                  end,
