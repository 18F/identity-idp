...
RSpec.describe NDS::InputComponent, type: :component do
  ...
  subject(:rendered) do
    render_inline NDS::InputComponent.new(
      form:,
      attribute: :email,
      label: 'Email address',
      **options,
  ...
  context 'email' do
    ...
    it 'requires a multi-label domain so bare hostnames are invalid' do
      input = rendered.css('input').first

      expect(input['type']).to eq('email')
      expect(input['pattern']).to eq(NDS::InputComponent::EMAIL_PATTERN)
    end

    it 'keeps an explicit pattern override' do
      rendered = render_inline NDS::InputComponent.new(
        form:,
        attribute: :email,
        label: 'Email address',
        type: :email,
      ...
      expect(rendered.css('input').first['pattern']).to eq('custom@pattern')
    end
  end
  ...
  context 'with custom validation error messages' do
    ...
    it 'includes the override in the validation messages data attribute' do
      messages = JSON.parse(rendered.css('.usa-input').first['data-nds-validation-messages'])
  ...
  context 'with an error message' do
    ...
    it 'normalizes string aria keys without duplicating attributes' do
      ...
      expect(rendered).to have_css('input[aria-describedby="hint preview_email_nds_error"]')
  ...
  context 'phone with country selector' do
    ...
    it 'emits the usa-input phone contract classes' do
      expect(rendered).to have_css('.usa-input.usa-input--phone')
      expect(rendered).to have_css('.usa-input__phone-shell .usa-input__country-shell')
      expect(rendered).to have_css('svg.usa-input__country-icon[aria-hidden="true"]', visible: :all)
      expect(rendered).to have_css('select.usa-input__country[data-nds-phone-country]')
      expect(rendered).to have_css('.usa-input__phone-body input[type=tel][data-nds-phone-input]')
    end
  end

  it 'emits the enhanced control + floating label + error contract' do
    expect(rendered).to have_css('.usa-input__shell .usa-input__control--floating')
    expect(rendered).to have_css('.usa-input__floating-label[aria-hidden="true"]')
    expect(rendered).to have_css('.usa-input__error[data-nds-error] .usa-input__error-inner')
  end

  context 'password toggle contract' do
    let(:options) { { type: :password, attribute: :password, label: 'Password' } }

    it 'emits the password toggle with show/hide icon hooks' do
      expect(rendered).to have_css('.usa-input.usa-input--password')
      expect(rendered).to have_css('button.usa-input__toggle[data-nds-password-toggle]')
      expect(rendered).to have_css(
        '.usa-input__toggle svg[data-nds-password-icon-show]',
        visible: :all,
      )
      expect(rendered).to have_css(
        '.usa-input__toggle svg[data-nds-password-icon-hide][hidden]',
        visible: :all,
      )
    end
  end
