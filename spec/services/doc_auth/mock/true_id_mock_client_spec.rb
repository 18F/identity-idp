...
RSpec.describe DocAuth::Mock::TrueIdMockClient do
  ...
  describe '#post_image' do
    context 'with bar code attention' do
      it 'succeeds to generaØte response indicate it' do
  ...
  it 'allows responses to be mocked' do
    ...
    expect(client.create_document).to eq('Create doc test')

    described_class.reset!

    expect(client.create_document).to_not eq('Create doc test')
  end

  context 'when checking results gives a failure' do
    before do
      DocAuth::Mock::TrueIdMockClient.mock_response!(
        method: :post_images,
