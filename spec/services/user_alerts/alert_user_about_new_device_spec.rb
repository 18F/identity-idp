...
RSpec.describe UserAlerts::AlertUserAboutNewDevice do
  ...
  describe '.send_alert' do
    ...
    before do
      allow(IdentityConfig.store).to receive(:new_device_alert_window_start_in_minutes).and_return(5)
    end
    ...
    context 'with sign in notification expired disavowal event' do
      ...
      context 'when there are authentication events within the time window' do
      ...
      context 'when there are no authentication events within the time window' do
        it 'logs an event and does not send email' do
          # 1. Exclude events outside possible window of time of recurring job run
          create(:event, user:, event_type: :sign_in_after_2fa, created_at: 16.minutes.ago)
          ...
          # 4. Exclude sign in notification timeframe expired event
          disavowal_event
          # No events after sign_in_notification_timeframe_expired

          analytics = FakeAnalytics.new
          allow(UserAlerts::AlertUserAboutNewDevice).to receive(:analytics).and_return(analytics)

          expect(UserMailer).not_to receive(:with)
          expect(result).to eq(true)
          expect(analytics).to have_logged_event(:new_device_alert_skipped)
        end
      end
    end
