...
RSpec.describe Proofing::LexisNexis::PhoneFinder::VerificationRequestRdpV3 do
  let(:applicant) do
    {
      uuid_prefix: '0987',
      uuid: '1234-abcd',
  ...
  describe '#body' do
    it 'returns a properly formed request body' do
      expect(subject.body).to eq(LexisNexisFixtures.phone_finder_rdp_v3_request_json)
    end
  ...
  describe '#url' do
    it 'returns a url for the Phone Finder endpoint' do
      expect(subject.url).to eq(
        'https://example.com/restws/identity/v3/accounts/test_account/workflows/customers.gsa2.phonefinder.workflow/conversations',
      )
    end
  end
