  ...
  describe '#decode_token' do
    ...
    context 'when communicating with piv/cac service' do
      context 'when in non-development mode' do
        ...
        describe 'when configured with a user-facing endpoint' do
          before(:each) do
            ...
            allow(Identity::Hostdata.settings).to receive(:piv_cac_service_url) { base_url }
          end
      ...
      describe 'when configured to contact piv_cac service for local development' do
        before(:each) do
          allow(Identity::Hostdata.settings).to receive(:identity_pki_local_dev).and_return('true')
          allow(Identity::Hostdata.settings).to receive(:identity_pki_disabled).and_return('false')
          allow(Identity::Hostdata.settings).to receive(:piv_cac_verify_token_url).
            and_return('http://localhost:8443/')
        end
